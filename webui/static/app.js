...
(function() {
  ...
  function connectDelegateEvents() {
    ...
    try {
      ...
      es.onmessage = (e) => {
        ...
        if (!payload || payload.type !== "delegate-finish")
          return;
  ...
  function normalizeFeatureFlags(payload) {
    const source = payload && typeof payload === "object" && payload.features && typeof payload.features === "object" ? payload.features : payload;
    return {
  ...
  function checkHealth() {
    ...
    if (token)
      opts.headers["Authorization"] = "Bearer " + token;
    fetch("/healthz", opts).then((r) => r.json()).then((d) => {
  ...
  function initLogin() {
    $("#login-btn").onclick = async () => {
      ...
      if (!t) {
        setMsg("#login-msg", "Please enter a token.", "error");
        return;
      }
      token = t;
      try {
  ...
  function initWelcome() {
    ...
    checkHealth();
    fetch("/healthz", { headers: token ? { Authorization: "Bearer " + token } : {} }).then((r) => r.json()).then((d) => {
      if (d.status === "ok") {
  ...
  function initSetup() {
    ...
    if (addProfile) {
      ...
      if (addProfile.hideWebhook) {
        ...
        if (webhookLabel)
          webhookLabel.classList.add("hidden");
      } else {
  ...
  async function initProvider() {
    ...
    if (heading)
      heading.textContent = isAddMode() ? "Step 2 \u2014 Select LLM Provider" : "Step 3 \u2014 Select LLM Provider";
    ...
    function renderCarrierDefaultChoice() {
      ...
      if (carrierDefaultProvider.reusable) {
        ...
        defaultSummary.textContent = "Using Carrier default: " + pname + " (`" + pid + "`) \xB7 credential: " + backend + ".";
    ...
    try {
      ...
      categoryOrder.forEach(({ key, label }) => {
        ...
        providers.forEach((p) => {
          ...
          li.innerHTML = "<strong>" + escapeHtml(p.name) + "</strong> <code>" + escapeHtml(p.id) + '</code> <span class="auth-badge">' + escapeHtml(badge) + "</span>";
          if (p.example_model) {
  ...
  function initConfig() {
    ...
    if (heading)
      heading.textContent = isAddMode() ? "Step 3 \u2014 Environment Variables" : "Step 4 \u2014 Environment Variables";
    let agentLabel = (isAddMode() ? "Adding: " : "Configuring: ") + selectedAgent;
    if (selectedProvider) {
      agentLabel += " \xB7 Provider: " + selectedProvider.name;
    }
    ...
    if (selectedProvider && selectedProvider.auth_mode === "api_key" && providerApiKey && selectedProvider.env_var) {
      ...
      k.type = "text";
      k.value = selectedProvider.env_var;
      k.placeholder = "KEY";
      const v = document.createElement("input");
      v.type = "password";
      v.value = providerApiKey;
      v.placeholder = "VALUE";
      row.appendChild(k);
      row.appendChild(v);
  ...
  function initComplete() {
    ...
    if (isAddMode() && lastAddResult) {
      ...
      if (pairRequired) {
        if (title)
          title.textContent = "\u26A0\uFE0F One step left: Pair your PicoClaw bot";
        lines.push("Action required: complete Telegram pairing before first use.");
      }
      if (lastAddResult.instanceId)
        lines.push("Instance: " + lastAddResult.instanceId);
  ...
  function renderQuickLaunchTemplateInputs(templateID) {
    ...
    schema.forEach((field) => {
      ...
      input.value = previous[key] != null ? String(previous[key]) : String(field && field.defaultValue ? field.defaultValue : "");
  ...
  function syncQuickLaunchModeUI() {
    ...
    if (templateField)
      templateField.classList.toggle("hidden", mode !== "template");
    if (mode === "template") {
  ...
  function renderQuickLaunchPlan(plan) {
    ...
    summary.textContent = "Approval: " + String(plan.approvalScope || "infrastructure_only") + (templateID ? " \xB7 Template: " + templateID : "") + " \xB7 Task units: " + String(Array.isArray(plan.taskUnits) ? plan.taskUnits.length : 0) + " \xB7 Max concurrency: " + String(plan.maxConcurrency || 0);
    tasks.textContent = "";
    workers.textContent = "";
    ...
    requiredWorkers.forEach((worker) => {
      ...
      row.textContent = hostTarget + "/" + String(worker.agentId || "zeroclaw") + " \xB7 count=" + String(worker.count || 1);
  ...
  async function runQuickLaunchExecution() {
    ...
    try {
      ...
      const blockedExecution = payload && payload.execution && typeof payload.execution === "object" ? payload.execution : null;
  ...
  async function initQuickLaunch() {
    ...
    if (modeSelect)
      modeSelect.onchange = () => syncQuickLaunchModeUI();
    if (templateSelect)
      templateSelect.onchange = () => renderQuickLaunchTemplateInputs(templateSelect.value);
    if (toggleBtn)
      toggleBtn.onclick = () => {
        if (!advanced)
          return;
  ...
  async function refreshInstances() {
    ...
    try {
      ...
      summary.textContent = "Total: " + instances.length + " \xB7 Running: " + running;
      instances.forEach((a) => {
        ...
        let metaText = "Type: " + agentID + " \xB7 Channel: " + channel + " \xB7 Provider: " + provider;
        if (pairRequired)
          metaText += " \xB7 Pair: required";
        else if (pairedChatId)
          metaText += " \xB7 Paired chat: " + pairedChatId;
  ...
  function executionStatusBadgeClass(status) {
    ...
    if (normalized === "partial_completed")
      return "badge badge-warn";
    if (normalized === "failed" || normalized === "retryable_failed" || normalized === "declined" || normalized === "cancelled")
  ...
  function applyExecutionRouteFilters(routeInfo) {
    ...
    const templateFilter = $("#executions-template-filter");
    const triggerFilter = $("#executions-trigger-filter");
    if (searchInput && typeof query.search === "string")
      searchInput.value = query.search;
    if (statusFilter && typeof query.status === "string")
      statusFilter.value = query.status;
    if (templateFilter && typeof query.template === "string")
      templateFilter.value = query.template;
    if (triggerFilter && typeof query.trigger === "string")
      triggerFilter.value = query.trigger;
  }
    ...
    if (!taskUnits.length) {
      ...
      taskUnits.forEach((task, index) => {
        ...
        if (failureReason || failureCategory) {
          ...
          const parts2 = [];
          if (failureReason)
            parts2.push("reason=" + failureReason);
          if (failureCategory)
            parts2.push("category=" + failureCategory);
          failure.textContent = parts2.join(" \xB7 ");
  ...
  function syncExecutionFilterOptions(executions) {
    ...
    if (templateFilter) {
      ...
      const templates = [...new Set((Array.isArray(executions) ? executions : []).map((item) => executionTemplateValue(item)).filter(Boolean))].sort((left, right) => left.localeCompare(right));
      templateFilter.innerHTML = '<option value="all">All</option>' + templates.map((value) => '<option value="' + escapeHtml(value) + '">' + escapeHtml(value) + "</option>").join("");
    ...
    if (triggerFilter) {
      ...
      const triggers = [...new Set((Array.isArray(executions) ? executions : []).map((item) => executionTriggerValue(item)).filter(Boolean))].sort((left, right) => left.localeCompare(right));
      triggerFilter.innerHTML = '<option value="all">All</option>' + triggers.map((value) => '<option value="' + escapeHtml(value) + '">' + escapeHtml(value) + "</option>").join("");
  ...
  function renderExecutionsDetailPanel(target, payload) {
    ...
    const outcome = execution && execution.outcome && typeof execution.outcome === "object" ? execution.outcome : {};
      ...
      if (artifacts.length) {
        ...
        artifacts.forEach((item) => {
          ...
          if (kind)
            metaParts.push(kind);
          if (contentType)
            metaParts.push(contentType);
          if (sizeBytes > 0)
            metaParts.push(String(sizeBytes) + " bytes");
          if (createdAt)
            metaParts.push(formatDateTime(createdAt));
          row.textContent = name + (metaParts.length ? " \xB7 " + metaParts.join(" \xB7 ") : "");
          outcomeCard.appendChild(row);
    ...
    if (policyDecision) {
      ...
      const toolPolicy = policy && policy.toolPolicy && typeof policy.toolPolicy === "object" ? policy.toolPolicy : {};
      ...
      if (matchedRuleName)
        policyLines.push("Matched rule: " + matchedRuleName);
      if (policyReason)
        policyLines.push("Reason: " + policyReason);
      if (toolMode)
        policyLines.push("tool mode: " + toolMode);
      if (configuredMaxConcurrency > 0)
        policyLines.push("configured concurrency: " + String(configuredMaxConcurrency));
      if (effectiveMaxConcurrency > 0)
        policyLines.push("effective concurrency: " + String(effectiveMaxConcurrency));
      if (maxTaskTimeoutMs > 0)
        policyLines.push("max task timeout: " + String(maxTaskTimeoutMs) + "ms");
      policyLines.push("max retry budget: " + String(maxRetryBudget));
      if (policySummary)
        policyLines.push("summary: " + policySummary);
      if (policyApprovedBy)
        policyLines.push("Approved by: " + policyApprovedBy);
      if (policyApprovedAt)
        policyLines.push("Approved at: " + formatDateTime(policyApprovedAt));
      ...
      if (targets.length) {
        ...
        targets.forEach((item) => {
          ...
          const row = document.createElement("div");
          row.className = "execution-detail-line";
          row.textContent = (host || (hostLabels.length ? "labels[" + hostLabels.join(",") + "]" : "local")) + "/" + agent + " \xB7 count=" + String(count);
          policyCard.appendChild(row);
        });
      }
    ...
    if (!providerResolutions.length) {
      ...
      providerResolutions.forEach((item) => {
        ...
        trace.forEach((traceItem) => {
          ...
          const traceProviderModel = [String(traceItem && traceItem.provider ? traceItem.provider : ""), String(traceItem && traceItem.model ? traceItem.model : "")].filter(Boolean).join("/");
  ...
  async function renderExecutionsView(executions, forceDetail) {
    ...
    if (!filtered.length) {
      ...
      filtered.forEach((execution) => {
        ...
        if (attributionParts.length) {
          ...
          attribution.textContent = attributionParts.join(" \xB7 ");
    ...
    try {
      ...
      exportEvidenceBtn.onclick = async () => {
        ...
        try {
          await downloadAPI(evidenceDownloadPath(selectedExecutionID), String(selectedExecutionID || "").trim() + "-evidence.zip");
        } catch (e) {
  ...
  function renderWorkersView(workers, summaryPayload, warnings) {
    ...
    summary.textContent = total ? "Total: " + total + " \xB7 Visible: " + filtered.length + " \xB7 Active: " + active + " \xB7 Busy: " + busy + " \xB7 Stale: " + stale + " \xB7 Active Executions: " + activeExecutions + " \xB7 Queued Tasks: " + queuedTasks + " \xB7 Local: " + local + " \xB7 Remote: " + remote : "No workers discovered yet.";
    ...
    filtered.forEach((worker) => {
      ...
      if (worker && worker.queuePosition)
        metaLines.push("queue position: " + String(worker.queuePosition));
      ...
      if (worker && worker.leaseState)
        metaLines.push("lease state: " + String(worker.leaseState));
      if (worker && worker.staleReason)
        metaLines.push("stale reason: " + String(worker.staleReason));
      if (worker && worker.leaseAgeSec)
        metaLines.push("lease age: " + formatAgeSeconds(worker.leaseAgeSec));
      if (worker && worker.heartbeatAgeSec)
        metaLines.push("heartbeat age: " + formatAgeSeconds(worker.heartbeatAgeSec));
      if (worker && worker.lastHeartbeatAt)
        metaLines.push("last heartbeat: " + formatDateTime(worker.lastHeartbeatAt));
  ...
  async function reclaimStaleWorkers() {
    ...
    try {
      ...
      const reclaim = payload && payload.reclaim ? payload.reclaim : {};
      await refreshWorkers();
      setMsg("#workers-msg", "Stale reclaim finished: reclaimed=" + String(Number(reclaim.reclaimed || 0) || 0) + ", skipped=" + String(Number(reclaim.skipped || 0) || 0) + ", failed=" + String(Number(reclaim.failed || 0) || 0), "info");
    } catch (e) {
  ...
  async function initWorkers() {
    ...
    if (reclaimStaleBtn)
      reclaimStaleBtn.onclick = reclaimStaleWorkers;
    if (searchInput)
      searchInput.oninput = () => renderWorkersView(workerInventoryCache, buildWorkerSummaryPayload(workerInventoryCache, workerQueueSummaryCache), []);
    if (stateFilter)
  ...
  function parseAgentStatus(text) {
    ...
    for (const line of lines) {
      ...
      if (m)
        result.push({ name: m[1], status: m[2].toLowerCase() });
    }
    return result;
  }
  ...
  function syncLogControls() {
    ...
    if (searchInput)
      searchInput.value = logSearchQuery;
  ...
  function parseLogLine(line) {
    ...
    if (!trimmed)
      return null;
    if (/^returned \d+ log lines for /i.test(trimmed))
      return null;
    let timestamp = "";
    let level = "UNKNOWN";
    let message = trimmed;
  ...
  function refreshLogStatus(visibleCount) {
    ...
    if (logPaused)
      parts.push("paused");
    if (logBuffer.length)
      parts.push("buffered " + logBuffer.length);
    status.textContent = parts.filter(Boolean).join(" \xB7 ");
  }
  function initChat() {
    showView("chat");
    $("#nav").classList.remove("hidden");
  ...
  function renderTriggerInputsText(inputs) {
    ...
    return Object.keys(source).sort((a, b) => a.localeCompare(b)).map((key) => String(key) + "=" + String(source[key] || "")).join("\n");
  }
  ...
  function beginTriggerEdit(triggerID) {
    ...
    const policyApprove = $("#trigger-policy-approve");
    if (policyApprove)
      policyApprove.checked = !!config.policyApprove;
    updateTriggerEditorUI();
  }
  ...
  function renderServerManageOperationMeta() {
    ...
    el.textContent = pieces.join(" \xB7 ");
  }
  ...
  function getServerManageSyncMode() {
    ...
    const value = input && input.value ? input.value.trim() : "";
    if (value === "always_push" || value === "manual")
      return value;
  ...
  function renderServerManageInstanceStatus(instance, steps) {
    ...
    if (health.toLowerCase() === "healthy" || runtimeState.toLowerCase() === "running") {
      badgeState = "ok";
      badgeText = "healthy";
    } else if (health.toLowerCase() === "unhealthy" || runtimeState.toLowerCase() === "error" || runtimeState.toLowerCase() === "stopped") {
      badgeState = "bad";
      badgeText = "unhealthy";
    }
    if (installed === true) {
    ...
    if (installed !== null)
      appendKV("install status", installed ? "installed" : "not installed");
    if (repaired !== null)
      appendKV("repair status", repaired ? "repaired" : "not repaired");
    if (gatewayHealthy !== null)
      appendKV("gateway", gatewayHealthy ? "healthy" : "unhealthy");
  ...
  async function loadServerManageConfig(options) {
    ...
    const task = async () => {
      ...
      if (!silent)
        setMsg("#server-manage-msg", "Loading config for host " + hostID + "...", "info");
      try {
  ...
  async function loadServerManageSessions(options) {
    ...
    const task = async () => {
      ...
      if (!silent)
        setMsg("#server-manage-msg", "Loading sessions for " + agentID + "...", "info");
      try {
  ...
  async function loadServerManageMemory(options) {
    ...
    const task = async () => {
      ...
      if (!silent)
        setMsg("#server-manage-msg", "Loading memory for " + agentID + "...", "info");
      try {
  ...
  async function loadServerManageInstances(options) {
    ...
    const task = async () => {
      ...
      if (!silent)
        setMsg("#server-manage-msg", "Loading instances for host " + hostID + "...", "info");
      try {
  ...
  async function syncServerManageInstance() {
    ...
    await runServerManageOperation("sync-instance", async () => {
      ...
      try {
        ...
        const payload = await serverManageAPI("POST", path, { mode }, "instance_sync");
  ...
  async function versionServerManageCodeAgent() {
    ...
    await runServerManageOperation("codeagent-version", async () => {
      ...
      try {
        ...
        renderServerManageInstanceStatus(payload.version || { backend }, []);
  ...
  function renderServersList(hosts) {
    ...
    const byID = new Map((Array.isArray(hosts) ? hosts : []).map((host) => [String(host && host.id ? host.id : ""), host]));
    async function handleCheck(hostID) {
      ...
      await runServerManageOperation("host-check", async () => {
        ...
        try {
          ...
          const pendingPull = checkPayload && Array.isArray(checkPayload.pendingPullInstances) ? checkPayload.pendingPullInstances : [];
          if (pendingPull.length > 0) {
            const pendingAgentIDs = pendingPull.map((instance) => pickRemoteInstanceAgentID(instance)).map((id) => String(id || "").trim()).filter(Boolean);
            const prompt = pendingAgentIDs.length > 0 ? "Discovered new remote instances (" + pendingAgentIDs.join(", ") + "). Pull configs to local machine now?" : "Discovered new remote instances. Pull configs to local machine now?";
    ...
    async function handleDelete(hostID) {
      ...
      if (!key)
        return;
      const host = byID.get(key) || {};
      if (!window.confirm("Delete remote host " + (host.name || host.id || key) + "?"))
        return;
      try {
    ...
    if (island && typeof island.renderServersList === "function" && island.renderServersList(wrap, hosts, {
      onCheck: handleCheck,
  ...
  async function initServers() {
    ...
    if (instanceStatusBtn)
      instanceStatusBtn.onclick = () => {
        loadServerManageInstanceStatus();
      };
    if (installInstanceBtn)
      installInstanceBtn.onclick = () => {
        installServerManageInstance();
      };
    if (repairInstanceBtn)
      repairInstanceBtn.onclick = () => {
        repairServerManageInstance();
      };
    if (loadLogsBtn)
      loadLogsBtn.onclick = () => {
        loadServerManageLogs();
      };
    if (syncInstanceBtn)
      syncInstanceBtn.onclick = () => {
        syncServerManageInstance();
      };
    if (syncStatusBtn)
      syncStatusBtn.onclick = () => {
        loadServerManageSyncStatus();
      };
    if (diagnoseInstanceBtn)
      diagnoseInstanceBtn.onclick = () => {
        diagnoseServerManageInstance();
      };
    if (reconcileInstanceBtn)
      reconcileInstanceBtn.onclick = () => {
        reconcileServerManageInstance();
      };
    if (rollbackInstanceBtn)
      rollbackInstanceBtn.onclick = () => {
        rollbackServerManageInstance();
      };
    if (codeAgentInstallBtn)
      codeAgentInstallBtn.onclick = () => {
        installServerManageCodeAgent();
      };
    if (codeAgentHealthBtn)
      codeAgentHealthBtn.onclick = () => {
        healthServerManageCodeAgent();
      };
    if (codeAgentVersionBtn)
      codeAgentVersionBtn.onclick = () => {
        versionServerManageCodeAgent();
      };
    if (codeAgentRunBtn)
      codeAgentRunBtn.onclick = () => {
        runServerManageCodeAgent();
      };
    if (loadConfigBtn)
      loadConfigBtn.onclick = () => {
        loadServerManageConfig();
      };
    if (applyConfigBtn)
      applyConfigBtn.onclick = () => {
        applyServerManageConfigPatch();
      };
    if (loadSessionsBtn)
      loadSessionsBtn.onclick = () => {
        loadServerManageSessions();
      };
    if (archiveSessionBtn)
      archiveSessionBtn.onclick = () => {
        applyServerManageSessionAction("archive");
      };
    if (deleteSessionBtn)
      deleteSessionBtn.onclick = () => {
        applyServerManageSessionAction("delete");
      };
    if (loadMemoryBtn)
  ...
  function renderProfilesAndBindings(profiles, bindings, policies) {
    ...
    if (!profilesWrap || !bindingsWrap || !policiesWrap)
      return;
    const profileByID = new Map((Array.isArray(profiles) ? profiles : []).map((profile) => [String(profile && profile.id ? profile.id : ""), profile]));
    const bindingByID = new Map((Array.isArray(bindings) ? bindings : []).map((binding) => [String(binding && binding.id ? binding.id : ""), binding]));
    const policyByID = new Map((Array.isArray(policies) ? policies : []).map((policy) => [String(policy && policy.id ? policy.id : ""), policy]));
    ...
    async function handleDeleteProfile(profileID) {
      ...
      if (!key)
        return;
      const profile = profileByID.get(key) || {};
      if (!window.confirm("Delete profile " + (profile.name || profile.id || key) + "?"))
        return;
      try {
    ...
    async function handleDeleteBinding(bindingID) {
      ...
      if (!window.confirm("Delete binding " + label + "?"))
        return;
      try {
    ...
    async function handleDeletePolicy(policyID) {
      ...
      if (!window.confirm("Delete execution policy " + label + "?"))
        return;
      try {
    ...
    const island = window.CarrierRemoteControlIslands;
    const renderedByIsland = !!(island && typeof island.renderProfilesAndBindings === "function" && island.renderProfilesAndBindings(profilesWrap, bindingsWrap, profiles, bindings, {
  ...
  function renderExecutionTriggers(triggers, onEditTrigger, onToggleTrigger, onDeleteTrigger) {
    ...
    list.forEach((trigger) => {
      ...
      const meta = document.createElement("div");
      meta.className = "instance-meta";
      meta.textContent = "id: " + (trigger.id || "-") + "\ntype: " + (trigger.type || "-") + "\ntemplate: " + (trigger.templateId || "-") + "\nenabled: " + String(trigger.enabled !== false) + (trigger.createdBy ? "\ncreated by: " + trigger.createdBy : "") + (config.provider ? "\nprovider: " + config.provider : "") + (config.hostIds && config.hostIds.length ? "\nhost ids: " + config.hostIds.join(", ") : "") + (config.hostLabels && config.hostLabels.length ? "\nhost labels: " + config.hostLabels.join(", ") : "") + (typeof config.maxConcurrency === "number" && config.maxConcurrency > 0 ? "\nmax concurrency: " + String(config.maxConcurrency) : "") + (config.policyApprove ? "\npolicy approve: true" : "") + (config.webhookSecretConfigured ? "\nwebhook secret: configured" : "") + (config.githubCommand ? "\ngithub command: " + config.githubCommand : "") + (config.githubLabel ? "\ngithub label: " + config.githubLabel : "") + (config.githubRepository ? "\ngithub repository: " + config.githubRepository : "") + (config.cron ? "\ncron: " + config.cron : "") + (config.timezone ? "\ntimezone: " + config.timezone : "") + (trigger.nextRunAt ? "\nnext run: " + trigger.nextRunAt : "") + (typeof trigger.triggeredCount === "number" && trigger.triggeredCount > 0 ? "\ntriggered count: " + String(trigger.triggeredCount) : "") + (trigger.lastExecutionId ? "\nlast execution: " + trigger.lastExecutionId : "") + (trigger.lastError ? "\nlast error: " + trigger.lastError : "") + (config.inputs && Object.keys(config.inputs).length ? "\ninputs:\n" + renderTriggerInputsText(config.inputs) : "");
      meta.style.whiteSpace = "pre-line";
      card.appendChild(title);
      card.appendChild(meta);
      if (canManagePoliciesUI()) {
  ...
  function parseCommaSeparatedValues(raw) {
    return String(raw || "").split(",").map((item) => String(item || "").trim()).filter(Boolean);
  }
  function renderGovernancePreviewResolution(payload) {
    ...
    trace.forEach((item) => {
      ...
      const providerModel = [String(item && item.provider ? item.provider : ""), String(item && item.model ? item.model : "")].filter(Boolean).join("/");
  ...
  async function initProfiles() {
    ...
    function syncTriggerControls() {
      ...
      if (saveTriggerBtn)
        saveTriggerBtn.disabled = !canManageTriggers;
      [
        "#trigger-name",
        "#trigger-type",
        "#trigger-template-id",
    ...
    async function refreshAll() {
      try {
        ...
        renderExecutionTriggers(triggers, (triggerID) => {
          ...
          const trigger = executionTriggersCache.find((item) => String(item && item.id ? item.id : "") === key) || {};
          const label = String(trigger && trigger.name ? trigger.name : key);
          if (!window.confirm("Delete execution trigger " + label + "?"))
            return;
          try {
    ...
    if (saveTriggerBtn)
      saveTriggerBtn.disabled = !canManagePoliciesUI();
  ...
  function normalizeRemoteMetricsPayload(payload) {
    ...
    const rolloutReasons = Array.isArray(rolloutSource.reasons) ? rolloutSource.reasons.map((item) => String(item || "")).filter(Boolean) : [];
    const rollout = {
  ...
  function topUsageAttribution(providerMetrics, key) {
    const attribution = providerMetrics && providerMetrics.attribution && typeof providerMetrics.attribution === "object" ? providerMetrics.attribution : {};
  ...
  function classifyRemoteOperationGroup(operationName) {
    ...
    if (name.startsWith("config_"))
      return "config";
    if (name.startsWith("session_") || name.startsWith("sessions_"))
      return "sessions";
    if (name.startsWith("memory_"))
      return "memory";
    if (name.startsWith("provider_"))
      return "provider";
    if (name.startsWith("remote_chat_") || name.startsWith("chat_"))
      return "chat";
    return "other";
  }
  ...
  function getFilteredRemoteOperationEntries(normalized) {
    ...
    const filtered = entries.filter(([name, stats]) => {
      ...
      if (selectedGroup !== "all" && group !== selectedGroup)
        return false;
      if (anomaliesOnly && !isRemoteOperationAnomaly(stats))
        return false;
      return true;
    });
    return {
  ...
  function renderRemoteObservabilityOperations(normalized) {
    ...
    const island = window.CarrierRemoteObservabilityIsland;
    if (!entries.length) {
      const emptyMessage = filtered.totalCount > 0 ? "No remote operation metrics match current filters." : "No remote operation metrics yet.";
      if (island && typeof island.renderOperations === "function") {
        island.renderOperations(body, { rows: [], emptyMessage });
      } else {
        body.textContent = "";
    ...
    if (island && typeof island.renderOperations === "function") {
      island.renderOperations(body, { rows });
      return filtered;
    }
    body.textContent = "";
    rows.forEach((row) => {
  ...
  async function initSettings() {
    ...
    try {
      ...
      if (info && info.selected_mode) {
        ...
        if (info.reason_code)
          summary += " (reason: " + info.reason_code + ")";
        lines.push(summary);
        if (info.hint)
          lines.push("Hint: " + info.hint);
      }
    } catch (_) {
    ...
    if (featureFlags.remoteControlPlaneEnabled) {
      try {
        ...
        lines.push("Remote metrics: ops=" + totalOps + ", op success rate=" + formatPercent(totals.successRate) + ", repair triggered=" + toFiniteNumber(repair.triggered, 0) + ", repair success=" + formatPercent(repair.successRate) + ", chat stream failure=" + formatPercent(chat.failureRate) + ".");
        lines.push("Remote rollout gate: state=" + rolloutState + ", can promote=" + (toFeatureBool(rollout.canPromote, false) ? "yes" : "no") + (rolloutReasons.length ? ", reasons=" + rolloutReasons.join("; ") : "") + ".");
      } catch (_) {
  ...
  function init() {
    ...
    if (token) {
      fetch("/healthz", { headers: { Authorization: "Bearer " + token } }).then((r) => {
        if (r.ok) {
          hideLogin();

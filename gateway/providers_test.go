...
func TestParseDiscordPayload_NonCommandReturnsNil(t *testing.T) {
	payload := map[string]interface{}{
		"id":         "msg-3",
		"channel_id": "ch-non-command",
		"content":    "hello",
	}
	if got := ParseDiscordPayload(payload); got != nil {
		t.Fatalf("expected nil normalized command for non-command payload, got %+v", got)
	}
}

func TestParseFeishuEvent_NonCommandReturnsNil(t *testing.T) {
	payload := map[string]interface{}{
		"header": map[string]interface{}{
			"event_id": "ev-no-command",
		},
		"event": map[string]interface{}{
			"message": map[string]interface{}{
				"chat_id": "chat-789",
	...
	if got := ParseFeishuEvent(payload); got != nil {
		t.Fatalf("expected nil normalized command for non-command event, got %+v", got)
	}
}

func TestParseTelegramUpdate_MissingFieldsReturnsNil(t *testing.T) {
	payload := map[string]interface{}{
		"update_id": float64(123),
		"message": map[string]interface{}{
			"text": "/agents",
			"chat": map[string]interface{}{}, // no chat.id
		},
	}
	if got := ParseTelegramUpdate(payload); got != nil {
		t.Fatalf("expected nil normalized command for malformed update, got %+v", got)
	}
}
...
func TestRenderTelegramResponse_HandoffAndDownloadBranches(t *testing.T) {
	resp := GatewayResponse{
		Result:        "ok",
		Message:       "diagnosis created",
		HandoffID:     "handoff-1",
		HandoffStatus: "",
		DownloadURL:   "/downloads/dl-1/archive.zip",
	}
	rendered := RenderTelegramResponse(resp)
	text, _ := rendered["text"].(string)
	if !strings.Contains(text, "Handoff: handoff-1 (pending)") {
		t.Fatalf("expected pending handoff text, got %q", text)
	}
	if !strings.Contains(text, "Download: /downloads/dl-1/archive.zip") {
		t.Fatalf("expected download line, got %q", text)
	}
	if v, ok := rendered["disable_web_page_preview"].(bool); !ok || !v {
		t.Fatalf("expected disable_web_page_preview=true, got %#v", rendered["disable_web_page_preview"])
	}
}

func TestRenderTelegramResponse_ErrorDefaultCode(t *testing.T) {
	resp := GatewayResponse{
		Result:  "error",
...
func TestRenderDiscordResponse_ErrorAndHandoffBranches(t *testing.T) {
	okResp := GatewayResponse{
		Result:    "ok",
		Message:   "handoff created",
		HandoffID: "handoff-2",
	}
	okRendered := RenderDiscordResponse(okResp)
	okContent, _ := okRendered["content"].(string)
	if !strings.Contains(okContent, "Handoff: handoff-2 (pending)") {
		t.Fatalf("expected default pending handoff in discord response, got %q", okContent)
	}

	errResp := GatewayResponse{
		Result:  "error",
		Message: "bad request",
	}
	errRendered := RenderDiscordResponse(errResp)
	errContent, _ := errRendered["content"].(string)
	if !strings.Contains(errContent, "E_UNKNOWN") {
		t.Fatalf("expected E_UNKNOWN default code in discord response, got %q", errContent)
	}
}

func TestRenderFeishuResponse_OKAndDefaultErrorCodeBranches(t *testing.T) {
	okResp := GatewayResponse{
		Result:      "ok",
		Message:     "done",
		DownloadURL: "/downloads/dl-2/file.zip",
	...
	okRendered := RenderFeishuResponse(okResp)
	okContent, _ := okRendered["content"].(map[string]interface{})
	okText, _ := okContent["text"].(string)
	if !strings.Contains(okText, "Download: /downloads/dl-2/file.zip") {
		t.Fatalf("expected download line in feishu ok response, got %q", okText)
	}
	if !strings.Contains(okText, "Handoff: handoff-3 (pending)") {
		t.Fatalf("expected pending handoff in feishu ok response, got %q", okText)
	}
	...
	errRendered := RenderFeishuResponse(errResp)
	errContent, _ := errRendered["content"].(map[string]interface{})
	errText, _ := errContent["text"].(string)
	if !strings.Contains(errText, "E_UNKNOWN") {
		t.Fatalf("expected default E_UNKNOWN in feishu error response, got %q", errText)
	}
}

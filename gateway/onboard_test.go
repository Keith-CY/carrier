...
func TestOnboardSelectProvider_OpenAICompatible_RequestsAPIKey(t *testing.T) {
	s := NewOnboardStore()
	key := "telegram:3"
	s.start(key)
	s.update(key, func(sess *OnboardSession) {
	...
	sess := s.get(key)
	if sess.Step != OnboardProviderSelected {
		t.Errorf("openai-compatible should remain on provider_selected, got %q", sess.Step)
	}
	if !strings.Contains(strings.ToLower(resp.Message), "api key") {
		t.Errorf("expected API key message, got: %q", resp.Message)
	}
}

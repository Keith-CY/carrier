...
func (*mockDaemonServer) Close() {}

type daemonRoundTripper struct {
	handler http.Handler
}

func (rt daemonRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	rr := httptest.NewRecorder()
	rt.handler.ServeHTTP(rr, req)
	return rr.Result(), nil
}

// newMockDaemonHandler creates a handler that mimics the daemon API.
// The handler map maps "METHOD /path" → handler func.
func newMockDaemonHandler(handlers map[string]http.HandlerFunc) http.Handler {
...
func newMockDaemon(handlers map[string]http.HandlerFunc) *httptest.Server {
	handler := newMockDaemonHandler(handlers)
...
func setupTestEnv(t *testing.T, handlers map[string]http.HandlerFunc) (*mockDaemonServer, *DaemonClient, *SessionStore, *DownloadStore, *OnboardStore) {
	t.Helper()
	handler := newMockDaemonHandler(handlers)
	dc := NewDaemonClient("http://daemon.local", "test-token", 5*time.Second)
	dc.httpClient.Transport = daemonRoundTripper{handler: handler}
	sessions := NewSessionStore("", 0, nil)
	t.Cleanup(sessions.Stop)
	downloads := NewDownloadStore("", nil)
	onboard := NewOnboardStore()
	return &mockDaemonServer{}, dc, sessions, downloads, onboard
}

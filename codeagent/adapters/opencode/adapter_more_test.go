...
func TestAdapterDefaultsAndNoopMethods(t *testing.T) {
	...
	if a.binary != "opencode" {
		t.Fatalf("expected default binary opencode, got %q", a.binary)
	}
	if a.runner == nil {
...
func TestAdapterHealthAndVersionBranches(t *testing.T) {
	...
	t.Run("version from stdout", func(t *testing.T) {
		a := NewAdapter(Options{Runner: func(_ context.Context, _ string, _ []string) (RunResult, error) {
			return RunResult{Stdout: " opencode v1.2.3 "}, nil
		}})
		version, err := a.Version(context.Background())
		if err != nil {
			t.Fatalf("Version returned error: %v", err)
		}
		if version != "opencode v1.2.3" {
	...
	t.Run("version from stderr", func(t *testing.T) {
		a := NewAdapter(Options{Runner: func(_ context.Context, _ string, _ []string) (RunResult, error) {
			return RunResult{Stderr: " opencode v1.2.4 "}, nil
		}})
		version, err := a.Version(context.Background())
		if err != nil {
			t.Fatalf("Version returned error: %v", err)
		}
		if version != "opencode v1.2.4" {
			t.Fatalf("unexpected version %q", version)
		}
	})
	...
	t.Run("version empty output", func(t *testing.T) {
		...
		if _, err := a.Version(context.Background()); err == nil || !strings.Contains(err.Error(), "empty opencode version output") {
...
func TestAdapterPromptAndArgsBranches(t *testing.T) {
	...
	argsWithSession := a.buildExecArgs(contract.RunRequest{Capability: contract.CapabilityRunShell, Command: "ls", ResumeSessionID: "sess-1"}, true)
	if !containsArg(argsWithSession, "--session") {
		t.Fatalf("expected --session in args: %v", argsWithSession)
	}
	argsWithoutSession := a.buildExecArgs(contract.RunRequest{Capability: contract.CapabilityRunShell, Command: "ls", ResumeSessionID: "sess-1"}, false)
	if containsArg(argsWithoutSession, "--session") {
		t.Fatalf("did not expect --session in args: %v", argsWithoutSession)
	}
}
...
func TestAdapterRunWithRetryErrorBranches(t *testing.T) {
	...
	res, err := a.runWithRetry(context.Background(), []string{"run"})
	...
	if _, err := aNoRetry.runWithRetry(context.Background(), []string{"run"}); err == nil {
...
func TestDefaultRunnerBranches(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "run opencode command") {

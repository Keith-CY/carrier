// Package gui exposes the Gio-based desktop GUI as a callable Run function.
package gui
...
// Run launches the Gio desktop GUI. It blocks until the window is closed.
func Run() {
	go func() {
		var w app.Window
...
func (s *appState) layoutConnect(gtx layout.Context, th *material.Theme) layout.Dimensions {
	return layout.Flex{Axis: layout.Vertical, Spacing: layout.SpaceEnd}.Layout(gtx,
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return material.Editor(th, &s.urlEditor, "http://localhost:9090").Layout(gtx)
		}),
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return material.Editor(th, &s.tokenEditor, "token").Layout(gtx)
		}),
		layout.Rigid(layout.Spacer{Height: unit.Dp(16)}.Layout),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return material.Button(th, &s.btnConnect, "Connect").Layout(gtx)
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			if s.connErr == "" {
				return layout.Dimensions{}

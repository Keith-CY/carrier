...
| E02 | Gateway lifecycle E2E flows | `scripts/e2e-integration.sh`, `gateway/server_webhook_test.go` |
...
| E06 | Diagnose artifact + remote diagnosis consent APIs | `daemon/internal/api/server.go`, `daemon/internal/lifecycle/service.go`, `gateway/server.go` |
| E07 | Cross-provider parity tests | `gateway/providers_test.go`, `gateway/server_test.go` |
...
| E17 | Redaction baseline + tests | `docs/security/redaction-baseline.md`, `shared/redact/redact_test.go`, `gateway/redact_test.go` |
